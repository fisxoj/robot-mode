...
;; to reload the file. Now when you open a .robot file emacs automatically sets
...
(setq robot-mode-keywords
	...
	("comment  .*" . font-lock-comment-face)
        ;;
        ("^[[:blank:]]+\\(?:\\(?:[Aa]nd\\|\\(?:[Gg]iv\\|[TtWw]h\\)en\\)\\)" . font-lock-builtin-face)))
...
not set 4 spaces are used."
  ...
  (if indent-tabs-mode
      "\t"
    (make-string (if (and (boundp 'c-basic-offset) (numberp c-basic-offset))
		     c-basic-offset
                   4)
		 ?\s)))
...
This function is bound to \\[robot-mode-complete]."
  ...
  (let ((kw-regexp (robot-mode-make-kw-regexp kw-prefix)))
    (cl-flet ((normalize-candidate-kw (kw)
...
This function is bound to \\[robot-mode-newline]."
  ...
  (cl-flet ((inside-kw-definition ()
    ...
    (if (inside-kw-definition)
        (insert (concat "\n" (robot-indent)))
      (progn (remove-possible-empty-tab)
             (insert "\n")))))
